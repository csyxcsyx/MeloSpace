...
|步骤 4：用户核心业务闭环| 已完成 |完成搜索、播放记录、歌单、收藏等用户能力|歌单 API、收藏 API、最近播放 API；产出见 `docs/step-4/`|普通用户能完成搜索、建歌单、收藏、评论|
...
4. **再做歌单和用户行为**：歌单、收藏、最近播放是用户侧完整体验的核心。
...
* **基础层**：用户、歌手、专辑、歌曲、歌单、收藏、搜索、后台管理、数据库持久化、播放组件。
...
* [√] 沿用当前阿里云 Nginx + systemd + MySQL 部署口径完成复测。
* [√] 完成服务器公网 HTTP 和 SSH 只读复测。
* [√] 补全 README、部署说明、测试说明和验收记录。
* [√] 准备答辩演示脚本。

**门禁：**

* [√] 公网环境可通过 IP 和域名访问。
* [√] MySQL 服务和媒体目录保留，S7 本轮只读复测未执行重启演练。
* [√] 演示主线可连续跑完，无阻塞级 Bug。
...
5. 创建歌单：拖拽调整歌曲顺序。
6. 登录管理员：上传歌曲、封面、歌词。
7. 展示服务器部署：Nginx 域名访问、Docker Compose 服务运行、数据库持久化。

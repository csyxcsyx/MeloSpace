...
|步骤|核心目标|必须产出|进入下一步的门禁|
|-|-|-|-|
|步骤 1：范围确认与设计准备|锁定技术栈、MVP 边界、页面范围、素材来源、ER 图|技术选型说明、页面清单、ER 初稿、测试音乐素材|能说清楚数据表、页面、接口和演示主线|
|步骤 2：后端基础架构|搭好 Spring Boot、数据库连接、统一返回、异常处理、参数校验、登录权限|后端项目、基础表、Auth API、Admin 权限|注册登录可用，管理员接口能鉴权|
|步骤 3：音乐内容管理闭环|完成歌曲、歌手、专辑、上传、上架/下架等后台能力|歌曲管理 API、上传接口、资源 URL 访问|管理员上传歌曲后，前台接口能查到并返回播放地址|
|步骤 4：用户核心业务闭环|完成搜索、播放记录、歌单、收藏、评论等用户能力|歌单 API、收藏 API、评论 API、最近播放 API|普通用户能完成搜索、建歌单、收藏、评论|
|步骤 5：前端主流程与播放器|完成主页面、全局播放器、接口联调、基础响应式|Vue 页面、播放器状态管理、核心页面联调|用户能完成“搜索 → 播放 → 收藏 → 建歌单 → 评论”|
|步骤 6：演示亮点增强|只做 2-3 个最稳的创新点|歌词高亮、场景推荐、拖拽歌单或沉浸播放页|亮点能在无报错情况下连续演示|
|步骤 7：测试、部署与答辩材料|修 Bug、补文档、部署演练、准备演示脚本|Docker Compose、Nginx 配置、测试清单、项目文档、演示材料|新环境可启动，答辩流程可连续跑完|
...
7. **提前部署，不要最后才部署**：后端和前端主流程跑通后，应尽快完成一次本地 Docker Compose 演练；正式答辩前再完成服务器或等价环境演练。
...
* 确定最终技术栈和项目目录。
* 输出页面清单：首页、搜索页、歌曲详情页、歌单详情页、个人中心、后台管理、沉浸播放页可选。
* 完成 ER 图和核心表字段。
* 准备合法测试素材：音频、封面、歌词、素材来源说明。
* 写出第一版接口清单。

**门禁：**

* ER 图能覆盖注册登录、歌曲、歌单、收藏、评论、最近播放。
* 至少准备 5 首可演示音频。
* 明确哪些功能是 P0、P1、P2。
...
* 完成歌单创建、编辑、删除、添加歌曲、移除歌曲、排序接口。
* 完成收藏、取消收藏、评论、删除评论接口。
* 完成播放记录/最近播放接口。
* 完成搜索接口的基础联动：歌曲、歌手、专辑、歌单。
* 完成个人中心所需的收藏、歌单、最近播放数据接口。
...
* 选择性完成场景推荐或沉浸播放页。
* 统一核心页面视觉风格。
* 补充演示数据，让首页、榜单、推荐区域不空。

**门禁：**

* 至少 2 个创新交互能稳定演示。
* 页面风格统一，播放器始终容易找到。
* P2 功能若影响 P0 稳定，立即回滚或降级。
...
* 编写并执行功能测试清单。
* 执行权限、上传、播放、部署、数据持久化测试。
* 完成 Dockerfile、docker-compose.yml、Nginx 配置。
* 完成服务器或本地 Docker 部署演练。
* 补全 README、数据库说明、接口说明、部署说明、测试说明。
* 准备答辩演示脚本、截图或视频。

**门禁：**

* 新环境按文档 30 分钟内能启动。
* 容器重启后 MySQL 数据和媒体文件不丢失。
* 演示主线可连续跑完，无阻塞级 Bug。

...
|步骤 2：后端基础架构|未开始|搭好 Spring Boot、数据库连接、统一返回、异常处理、参数校验、登录权限|后端项目、基础表、Auth API、Admin 权限|注册登录可用，管理员接口能鉴权|
|步骤 3：音乐内容管理闭环|未开始|完成歌曲、歌手、专辑、上传、上架/下架等后台能力|歌曲管理 API、上传接口、资源 URL 访问|管理员上传歌曲后，前台接口能查到并返回播放地址|
|步骤 4：用户核心业务闭环|未开始|完成搜索、播放记录、歌单、收藏、评论等用户能力|歌单 API、收藏 API、评论 API、最近播放 API|普通用户能完成搜索、建歌单、收藏、评论|
|步骤 5：前端主流程与播放器|未开始|完成主页面、全局播放器、接口联调、基础响应式|Vue 页面、播放器状态管理、核心页面联调|用户能完成“搜索 → 播放 → 收藏 → 建歌单 → 评论”|
|步骤 6：演示亮点增强|未开始|只做 2-3 个最稳的创新点|歌词高亮、场景推荐、拖拽歌单或沉浸播放页|亮点能在无报错情况下连续演示|
|步骤 7：测试、部署与答辩材料|未开始|修 Bug、补文档、部署演练、准备演示脚本|Docker Compose、Nginx 配置、测试清单、项目文档、演示材料|新环境可启动，答辩流程可连续跑完|
